...
public class ModCraftResultSlot extends Slot {
    ...
    public @NotNull ItemStack remove(int pAmount) {
        if ( this.hasItem() ) this.removeCount += Math.min(pAmount, this.getItem().getCount());
    ...
    public void onTake(@NotNull Player player, @NotNull ItemStack stack) {
        ...
        NonNullList<ItemStack> remaining = player.level().getRecipeManager().getRemainingItemsFor(RuneCraftingRecipe.Type.INSTANCE, this.craftSlots, player.level());
        ...
        for ( int i = 0; i < remaining.size(); i++ ) {
            ...
            if ( !remainingStack.isEmpty() ) {
                if ( slotStack.isEmpty() ) this.craftSlots.setItem(i, remainingStack);
                ...
                else if ( !this.player.getInventory().add(remainingStack) ) this.player.drop(remainingStack, false);
            }
        }
    ...
    protected void checkTakeAchievements(@NotNull ItemStack pStack) {
        ...
        if ( this.container instanceof RecipeHolder recipeholder ) recipeholder.awardUsedRecipes(this.player, this.craftSlots.getItems());
        this.removeCount = 0;
    }
}
