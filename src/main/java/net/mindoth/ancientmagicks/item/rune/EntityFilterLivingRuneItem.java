...
public class EntityFilterLivingRuneItem extends UseOnEntityTemplate {
    public EntityFilterLivingRuneItem(Properties pProperties) {
        super(pProperties);
    }
        ...
        if ( !spellData.getBooleans().isEmpty() ) {
            if ( spellData.getLatestBoolean() != null ) {
                ...
                for ( Entity entry : result.getEntities() ) {
                    if ( bool ) {
                        if ( entry instanceof LivingEntity ) entities.add(entry);
                    }
                    else {
                        if ( !(entry instanceof LivingEntity) ) entities.add(entry);
                    }
